...
/*! \mainpage Scythe Statistical Library: Application Programmers'
 * Interface
 *
 * \section intro Introduction
 *
 * The Scythe Statistical Library is an open source C++ library for
 * statistical computation, written by Daniel Pemstein (North Dakota
 * State University), Kevin M. Quinn (University of California
 * Berkeley), and Andrew D.  Martin (Washington University). It
 * includes a suite of matrix manipulation functions, a suite of
 * pseudo-random number generators, and a suite of numerical
 * optimization routines.  Programs written using Scythe are generally
 * much faster than those written in commonly used interpreted
 * languages, such as R and MATLAB, and can be compiled on any system
 * with the GNU GCC compiler (and perhaps with other C++ compilers).
 * One of the primary design goals of the Scythe developers has been
 * ease of use for non-expert C++ programmers. We provide ease of use
 * through three primary mechanisms: (1) operator and function
 * over-loading, (2) numerous pre-fabricated utility functions, and
 * (3) clear documentation and example programs. Additionally, Scythe
 * is quite flexible and entirely extensible because the source code
 * is available to all users under the GNU General Public License.
 *
 ...
 * SCYTHE_COMPILE_DIRECT flag allows the user to compile Scythe
 * sources directly.  The SCYTHE_VIEW_ASSIGNMENT_FLAG flag turns on
 * R-style recycling in Matrix::operator=() for view matrices. 
 *
 * The SCYTHE_DEBUG flag controls the amount of error trapping in
 ...
 * and will generally include a line similar to \verbatim PKG_CPPFLAGS
 * = -DSCYTHE_COMPILE_DIRECT -DSCYTHE_RPACK \endverbatim in the
 * src/Makevars.in file within the package.
 * 
 * \section copy Copyright
 *
 * Scythe Statistical Library Copyright (C) 2000-2002 Andrew D. Martin
 * and Kevin M.  Quinn; 2002-2013 Andrew D. Martin, Kevin M. Quinn,
