...
namespace scythe {
   ...
   */
    ...
    public:
      ...
      */
      inline T_type asScalar () {
        SCYTHE_CHECK_10(! this->isScalar(), scythe_conformation_error,
            "Attempted to coerce non-scalar matrix to double");
       ...
       * nXm case, which forces a resize.
          ...
          std::transform(M.template begin_f<ORDER>(), 
              M.template end_f<ORDER>(), begin_f(), 
              std::bind1st(op, tmp));
				} else if (M.size() == 1) { // nXm += 1x1
					std::transform(begin_f(), end_f(), begin_f(),
							std::bind2nd(op, M(0)));
				} else { // nXm += nXm
            std::transform(begin_f(), end_f(), 
                M.template begin_f<ORDER>(), begin_f(), op);
        }
