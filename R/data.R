...
r_dir_prefix = ifelse(path[length(path)] == "R", "./", "./R/")
...
## so i just recoded 364 for those years in xpolity
## (4) Germany
...
ged = read.csv(paste0(data_prefix, "ged.csv")) %>%
...
rm(icews.2008, icews.2007, icews.2006, icews.2005)
gc()
...
data_list = list(gw, fariss, gtd, mid, ucdp_conflict, uds_xpolity,
  ksg, epr_exclpop, epr_oil, xpolity, ged, protest)
